  ...
  methods: {
    ...
    onFileChange (e) {
      ...
      if (files.length === 0) {
        ...
        fr.onload = (e) => { // onload等待把文件读成base64字符串会触发onload事件函数
        // e.target.result的值就是读完的结果（base64字符串）
    ...
    async uploadFn () {
      ...
      // 立刻让vuex里actions（获取用户信息的actions）再请求一次后台更新vuex里userInfo的值
