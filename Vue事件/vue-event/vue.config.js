...
module.exports = defineConfig({
  ...
  // node里有个内置的环境变量process.env.NOOE_ENV
  // process.env.NOOE_ENV 它会根据你敲击的命令，来使用不同的值
  // 解决：
  // 如果你敲击npm run serve, process.env.NOOE_ENV的值就是'development'字符串
  // 如果你敲击npm run build, process.env.NOOE_ENV的值就是'prodution'字符串
...
// 生产环境，打包时会以public/index.html作为模板生成dist/index.html中就有了那些cdn地址
// 开发环境，public/index.html，也有那些第三方的cdn地址，所以也能正常运行
// 但是第一次的速度有点慢，所以开发环境呢我还是想让webpack引入本地的node_modules那些第三方包
